<template>
    <div class=" dark:text-gray-100 border-2 border-gray-300 hover:border-red-600">
      <div class="sm:bg-red-700 rounded shadow-md lg:px-0 px-4">
        <img :src="product.image" :alt="product.title + ' image'"
          class="xl:object-scale-down xl:p-4  md:p-2 object-center w-full max-h-72 mx-auto rounded-t-md dark:bg-white m-8" />
      </div>
  
      <div class="flex flex-col justify-between p-6 space-y-8">
        <div>
          <h2 class="text-3xl font-semibold text-gray-800 truncate">{{ product.title }}</h2>
          <p class="text-gray-400 w-full">Curabitur luctus erat nunc, sed .</p>
        </div>
        <div class="flex flex-row flex-wrap justify-between items-center">
          <div class="flex flex-col justify-start dark:text-gray-500 w-full md:w-8/12">
            <p class="text-sm">test</p>
            <p class="text-sm">test</p>
          </div>
          <div class="w-full md:w-auto mt-4 md:mt-0">
            <button type="button"
              class="flex items-center justify-center w-full p-3 font-semibold tracking-tight rounded dark:bg-gray-400 dark:text-gray-900">
              Read more
            </button>
          </div>
        </div>
      </div>
    </div>
  </template>
  
  
  
<script setup lang="ts">
import { defineProps } from 'vue';

const { product } = defineProps<{ product: any }>();
</script>
  